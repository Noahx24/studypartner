    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input type="date" value={form.exam_date} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, exam_date: e.target.value })} className="mt-1" />
        </div>
        <div>
          <Label className="text-xs font-medium">Assignment Due</Label>
          <Input type="date" value={form.assignment_date} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, assignment_date: e.target.value })} className="mt-1" />
        </div>
      </div>
      <div>
        <Label className="text-xs font-medium">Estimated Pages</Label>
        <Input type="number" placeholder="e.g. 80" value={form.estimated_pages} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, estimated_pages: e.target.value })} className="mt-1" />
      </div>
        ...
        <div>
          ...
          {showUnits && (
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {(analysis.units || []).map((unit: any, i: number) => (
...
Apply any student feedback corrections strictly.`,
  ...
  const handleSave = async () => {
    ...
    const materialData = {
      ...form,
      estimated_hours: analysis?.estimated_hours || 0,
      complexity: analysis?.complexity || 'moderate',
      topics_summary: analysis?.topics_summary || '',
      estimated_pages: form.estimated_pages ? Number(form.estimated_pages) : analysis?.estimated_pages || 0,
