...
export function FeedbackModal({ open, estimated, onClose, onSubmit, }) {
    const [actual, setActual] = useState(estimated);
    if (!open)
        return null;
    return (<div className="fixed inset-0 z-40 flex items-end justify-center bg-slate-900/30 p-4 md:items-center">
      <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl">
        ...
        <input className="input mt-1" type="number" min={1} value={actual} onChange={(event) => setActual(Number(event.target.value))}/>
