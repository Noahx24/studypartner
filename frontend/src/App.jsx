...
export function App() {
    ...
    return (<main className="relative mx-auto min-h-screen w-full max-w-[440px]" style={{ background: P.bg }}>
      {error && (<div className="fixed left-1/2 top-2 z-50 flex -translate-x-1/2 items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold" style={{ background: P.coralSoft, color: P.coralDeep }}>
          {error}
          <button onClick={() => setError(null)} className="opacity-60">×</button>
        </div>)}

      {route.name === 'home' && (<DashboardView user={user} modules={modules} assessments={assessments} todaySessions={todaySessions} weekSessions={weekSessions} syncState={syncState === 'online' && outboxCount > 0 ? { queued: outboxCount } : syncState} onSync={handleSync} onStartSession={(s) => setRoute({ name: 'timer', session: s })} onOpenModules={() => setRoute({ name: 'modules' })} onOpenAssessments={() => setRoute({ name: 'assessments' })} onOpenModule={(id) => {
                void fetchModuleDetails(id);
                setRoute({ name: 'module_detail', moduleId: id });
            }} onOpenSettings={() => setRoute({ name: 'settings' })}/>)}

      {route.name === 'settings' && (<section className="px-4 pb-24 pt-12">
      ...
      {route.name === 'today' && (<TodayView user={user} modules={modules} sessions={todaySessions} onStartSession={(s) => setRoute({ name: 'timer', session: s })} onCompleteSession={handleComplete} onReschedule={async () => {
                await api.reschedule({ user_id: user.userId });
                await loadPlans(user.userId);
            }}/>)}

      {route.name === 'week' && (<WeekView modules={modules} weekSessions={weekSessions} assessments={assessments}/>)}

      {route.name === 'calendar' && (<CalendarView modules={modules} weekSessions={weekSessions} assessments={assessments} onOpenAssessments={() => setRoute({ name: 'assessments' })}/>)}

      {route.name === 'modules' && (<ModulesView modules={modules} weekSessions={weekSessions} assessments={assessments} onOpenModule={(id) => {
                void fetchModuleDetails(id);
                setRoute({ name: 'module_detail', moduleId: id });
            }} onUpload={() => setRoute({ name: 'upload' })}/>)}

      {route.name === 'module_detail' && (<ModuleDetailView module={moduleById[route.moduleId]} detail={moduleDetails[route.moduleId]} assessments={assessments.filter((a) => a.module_id === route.moduleId)} onBack={() => setRoute({ name: 'modules' })} onPlan={() => setRoute({ name: 'selection', moduleId: route.moduleId })} onPacks={() => setRoute({ name: 'packs', moduleId: route.moduleId, selectionId: activeSelectionId ?? undefined })} onUpload={() => setRoute({ name: 'upload', moduleId: route.moduleId })}/>)}

      {route.name === 'assessments' && (<AssessmentsView modules={modules} assessments={assessments} onBack={() => setRoute({ name: 'calendar' })} onAdd={addAssessment}/>)}

      {route.name === 'timer' && (<TimerView session={route.session} module={moduleById[route.session.module_id]} onClose={() => setRoute({ name: 'today' })} onComplete={async () => {
                await handleComplete(route.session);
                setRoute({ name: 'today' });
            }}/>)}

      {route.name === 'upload' && (<UploadView modules={modules} moduleId={route.moduleId} userId={user.userId} onBack={() => setRoute({ name: 'modules' })} onUploaded={async (moduleId) => {
                await fetchModuleDetails(moduleId);
                await loadPlans(user.userId);
            }}/>)}

      {route.name === 'selection' && (<SelectionView userId={user.userId} moduleId={route.moduleId} onBack={() => setRoute({ name: 'module_detail', moduleId: route.moduleId })} onSelectionSaved={(sid) => {
                setActiveSelectionId(sid);
                setRoute({ name: 'packs', moduleId: route.moduleId, selectionId: sid });
            }}/>)}

      {route.name === 'packs' && (<StudyPacksView userId={user.userId} moduleId={route.moduleId} activeSelectionId={route.selectionId ?? activeSelectionId} onBack={() => setRoute({ name: 'module_detail', moduleId: route.moduleId })} onOpenPack={(pid, payload) => setRoute({ name: 'reader', packId: pid, payload })}/>)}

      {route.name === 'reader' && (<PackReaderView pack_id={route.packId} payload={route.payload} onClose={() => setRoute({ name: 'modules' })}/>)}
