...
export function SelectionView({ userId, moduleId, onBack, onSelectionSaved }: Props) {
  ...
  const toggleLU = (luId: string) => {
    ...
    setSelected((prev) => {
      const next = new Set(prev);
      for (const s of lu.subtopics) (allIn ? next.delete(s.id) : next.add(s.id));
      return next;
    });
  };

  const toggleExpand = (luId: string) =>
    setExpanded((prev) => {
      const next = new Set(prev);
  ...
  return (
    <Screen>
        ...
        {!structure ? (
          ...
          <>
                ...
                return (
                  <Card key={lu.id} pad={0}>
                    <div className="p-[14px]">
                      <div className="flex items-center gap-3">
                        ...
                        >
                          {allIn ? (
                            ...
                            <div className="h-[2px] w-3" style={{ background: P.ink }} />
                          ) : null}
                        </button>
                      ...
                      {isOpen && (
                        <ul className="ml-9 mt-3 space-y-1.5">
                          {lu.subtopics.map((s) => (
                            <li key={s.id}>
                              <button
                              ...
                              >
                                ...
                                <span
                                  className="mono ml-auto text-[11px] text-ink3"
                                  style={{ fontFamily: MONO }}
                                >
                                  {s.word_count}w
                                </span>
            ...
            <Card>
              <Toggle
                label="Summaries"
                sub="Key concepts, bullets, simple explanation"
                value={features.summaries}
                onChange={(v) => setFeatures((f) => ({ ...f, summaries: v }))}
              />
              <Toggle
                label="Subtopic quizzes"
                sub="3–5 questions per subtopic"
                value={features.subtopic_quiz}
                onChange={(v) => setFeatures((f) => ({ ...f, subtopic_quiz: v }))}
              />
              <Toggle
                label="Topic-level quiz"
                sub="Revision quiz at the end of each unit"
                value={features.topic_quiz}
                onChange={(v) => setFeatures((f) => ({ ...f, topic_quiz: v }))}
              />
              <Toggle
                last
                label="Low-data mode"
                sub="Shorter outputs, text only"
                value={lowData}
                onChange={setLowData}
              />
            </Card>
      ...
      <div
        className="fixed bottom-0 left-1/2 z-30 w-full max-w-[440px] -translate-x-1/2 px-4 pb-7 pt-3"
        ...
        <Card variant="dark" pad={12}>
          <div className="flex items-center gap-3">
            <div>
              ...
              <div
                className="mono text-[18px] font-bold text-white"
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{ background: value ? P.ink : P.line }}
        aria-pressed={value}
      >
        <span
