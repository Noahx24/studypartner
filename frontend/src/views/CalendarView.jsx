...
export function CalendarView({ modules, weekSessions, assessments, onOpenAssessments }) {
    ...
    const monthLabel = today
        ...
        .toUpperCase();
    const upcoming = useMemo(() => [...assessments].sort((a, b) => a.due_date.localeCompare(b.due_date)).slice(0, 3), [assessments]);
    return (<Screen>
      <ScreenHeader subtitle={monthLabel} title="Calendar" right={<IconBtn aria-label="Filter">
            <Icon name="filter" size={18}/>
          </IconBtn>}/>

      <div className="px-4">
        <div className="grid grid-cols-7 gap-1 px-1 pb-2">
          {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (<div key={i} className="mono text-center text-[11px] font-bold text-ink3" style={{ fontFamily: MONO }}>
              {d}
            </div>))}
        </div>
        ...
        <Card pad={8}>
          <div className="grid grid-cols-7 gap-1">
            ...
            return (<div key={i} className="relative flex aspect-square flex-col rounded-[10px] p-1.5" style={{
                    background: isToday ? P.ink : 'transparent',
                    color: isToday ? P.surface : P.ink,
                }}>
                  <div className="mono text-[13px]" style={{
                    fontFamily: MONO,
                  ...
                  <div className="mt-auto flex items-end gap-0.5">
                    {Array.from({ length: Math.min(load, 4) }).map((_, j) => (<div key={j} className="h-[3px] flex-1 rounded" style={{
                        background: isToday ? P.lime : P.primary,
                        opacity: isToday ? 1 : 0.4 + j * 0.15,
                    }}/>))}
                  </div>
                  {c && (<div className="absolute right-1 top-1 h-2 w-2 rounded-full" style={{
                        background: c.solid,
        ...
        <Card pad={14}>
          <div className="flex flex-wrap gap-3">
            ...
            return (<div key={m.id} className="mono flex items-center gap-2 text-xs text-ink2" style={{ fontFamily: MONO }}>
