...
export function WeekView({ modules, weekSessions, assessments }: Props) {
        ...
        <Card pad={16} style={{ marginBottom: 14 }}>
          <div className="mb-3 flex items-center justify-between">
            <div>
              ...
              <div
                className="mono text-[22px] font-bold tracking-[-0.6px] text-ink"
            ...
            {days.map((d) => {
              ...
              return (
                <div key={d.key} className="flex flex-col items-center gap-1.5">
                  <div
                    className="mono text-[10px] font-semibold text-ink3"
                    style={{ fontFamily: MONO }}
                  >
                    {d.label[0]}
                  </div>
                ...
                {d.sessions.map((s) => {
                  ...
                  return (
                    <WeekSessionRow
                      key={s.id}
                      session={s}
                      moduleName={mod?.name ?? s.module_id}
                    />
                  );
                })}
              </div>
...
function WeekSessionRow({
  ...
  return (
    <div
      className="flex items-center gap-2.5 rounded-[14px] border border-line bg-surface px-3.5 py-3"
      style={{ opacity: done ? 0.55 : 1 }}
    >
      <div
        className="h-[30px] w-1 shrink-0 rounded"
      ...
      <div
        className="mono text-[12px] font-semibold text-ink2"
        style={{ fontFamily: MONO }}
      >
