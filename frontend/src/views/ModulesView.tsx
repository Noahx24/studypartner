...
export function ModulesView({ modules, weekSessions, assessments, onOpenModule, onUpload }: Props) {
  const stats = useMemo(
    () =>
      modules.map((m) => {
  ...
  return (
    <Screen>
      <ScreenHeader
        ...
        title="Modules"
        right={
          <IconBtn onClick={onUpload} aria-label="Upload">
        ...
        {stats.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {stats.map((m) => {
              ...
              const statusLabel =
                m.status === 'risk' ? 'Critical' : m.status === 'warn' ? 'Behind' : 'On track';
              return (
                <Card key={m.id} onClick={() => onOpenModule(m.id)} pad={0}>
                  <div className="p-[18px]">
                    <div className="mb-3.5 flex items-center gap-3">
                      <div
                        className="mono flex h-11 w-11 items-center justify-center rounded-[12px] text-[11px] font-bold"
                    ...
                    <div className="grid grid-cols-3 gap-2.5">
                      ...
                      <StatCell
                        label="Type"
                        value={m.module_type === 'year' ? 'Year' : 'Term'}
                      />
                    </div>

                    <div
                      className="mt-3.5 h-2 overflow-hidden rounded"
                      style={{ background: P.line }}
                    >
                      <div
                        className="h-full rounded"
                        style={{ width: `${m.progress}%`, background: c.solid }}
                      />
                    </div>
                    <div
                      className="mono mt-1.5 flex justify-between text-[11px] text-ink3"
