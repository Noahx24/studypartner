...
import type { OnboardingData, ModuleForm } from '../types';

type Props = { onDone: (data: OnboardingData) => Promise<void> };
...
export function LandingView({ onDone }: Props) {
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-[440px] flex-col" style={{ background: P.bg }}>
      <div className="flex gap-[6px] px-6 pt-[62px]">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-1 flex-1 rounded-full"
            style={{ background: i <= step ? P.ink : P.line }}
          />
        ))}
      </div>
      ...
      <div className="px-6 pt-6">
        <button
          ...
          className="flex items-center gap-1 text-[13px] text-ink2"
        >
      ...
      {step === 1 && (
        <StepHours weekly={weekly} onChange={setWeekly} onNext={() => setStep(2)} />
      )}
      {step === 2 && (
        <StepWindows windows={windows} onChange={setWindows} onNext={() => setStep(3)} />
      )}
      {step === 3 && (
        <StepModules
          modules={modules}
...
function IntroStep({ onContinue }: { onContinue: () => void }) {
  ...
  return (
    <div className="flex min-h-screen flex-col px-6 pb-8 pt-[70px]" style={{ background: P.bg }}>
      <div className="relative mb-2 h-[220px]">
        {cards.map((c, i) => (
          <div
            key={i}
            className="absolute flex h-[60px] items-center rounded-2xl px-4 text-sm font-semibold"
            style={{
              top: i * 60 + 10,
...
function StepHours({
  ...
  return (
    <StepShell
      title="How much study time, each week?"
      sub="We'll never overbook. You can change this later."
      onNext={onNext}
      nextLabel="Continue"
    >
      <div className="py-3 text-center">
        <div
          className="mono font-semibold leading-none text-ink"
          style={{ fontSize: 88, letterSpacing: '-4px', fontFamily: MONO }}
        >
          {weekly}
      ...
      <div
        className="mt-6 flex gap-2.5 rounded-2xl p-3.5"
        style={{ background: P.primarySoft }}
      >
...
type WindowSet = { morning: boolean; lunch: boolean; evening: boolean; weekend: boolean };
...
function StepWindows({
  ...
  windows: WindowSet;
  onChange: (w: WindowSet) => void;
  onNext: () => void;
}) {
  const OPTIONS: Array<{ id: keyof WindowSet; label: string; sub: string; color: string }> = [
  ...
  return (
    <StepShell
      ...
      sub="Tap the windows that usually work. We'll schedule within them."
      onNext={onNext}
      nextLabel="Continue"
      disabled={!Object.values(windows).some(Boolean)}
    >
      <div className="mt-4 flex flex-col gap-2.5">
        {OPTIONS.map((w) => {
          ...
          return (
            <button
              key={w.id}
              onClick={() => onChange({ ...windows, [w.id]: !on })}
            ...
            >
              <div
                className="h-9 w-2.5 rounded"
                style={{ background: on ? w.color : P.line }}
              />
...
function StepModules({
  ...
  const [draftType, setDraftType] = useState<'year' | 'semester'>('semester');
  ...
  return (
    <StepShell
      title="Your modules this term"
      sub="We'll weight time based on deadlines and credit."
      onNext={onFinish}
      nextLabel="Generate my first week →"
      disabled={modules.length === 0 || loading}
    >
      <div className="mt-2.5 flex flex-col gap-2.5">
        {modules.map((m) => {
          ...
          return (
            ...
            >
              <div
                className="mono flex h-[42px] w-[42px] items-center justify-center rounded-[10px] text-[11px] font-bold"
        ...
        <div className="rounded-[14px] border border-dashed border-line bg-transparent p-3">
          ...
          <select
            className="input mb-2"
            value={draftType}
            onChange={(e) => setDraftType(e.target.value as 'year' | 'semester')}
          >
