...
import type { AssessmentForm, ModuleForm, StudySession, UserSettings } from '../types';
...
type Props = {
  ...
  syncState: 'online' | 'offline' | 'syncing' | { queued: number };
  onSync: () => void;
...
export function DashboardView({
  ...
  const moduleStats = useMemo(() => {
    return modules.map((m) => {
      ...
      const daysLeft = nextDeadline
        ...
        : null;
      const status: 'ok' | 'warn' | 'risk' =
        daysLeft !== null && daysLeft <= 7 && progress < 60
          ? 'risk'
  ...
  const upcoming = useMemo(
    () => [...assessments].sort((a, b) => a.due_date.localeCompare(b.due_date)).slice(0, 2),
    [assessments],
  );
  ...
  return (
    <Screen>
      <ScreenHeader
        subtitle={`${greet.toUpperCase()}, ${user.name.toUpperCase()}`}
        title={
          remainingMin > 0
      ...
      <div className="px-4">
        {nextSession ? (
          <NextSessionCard
            session={nextSession}
            moduleName={modules.find((m) => m.id === nextSession.module_id)?.name ?? nextSession.module_id}
            onStart={() => onStartSession(nextSession)}
          />
        ) : (
          <Card variant="dark">
            <div className="mb-2 flex items-center gap-2">
              ...
              <span
                className="mono text-[11px] font-bold uppercase tracking-wider opacity-70"
          ...
          <Card pad={16}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div
                  className="mono text-[22px] font-bold text-ink"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div
                  ...
                  style={{ color: P.limeInk, fontFamily: MONO, letterSpacing: '-1px' }}
                >
                  {doneMin}
        ...
        <div className="flex flex-col gap-2.5">
          {moduleStats.length === 0 ? (
            ...
            moduleStats.slice(0, 3).map((m) => (
              <ModuleProgressRow key={m.id} module={m} onClick={() => onOpenModule(m.id)} />
            ))
          )}
        </div>
  ...
  return (
    <Card variant="dark" pad={0}>
      <div className="relative p-5">
        ...
        <div
          className="mono mb-1 text-[13px] font-bold tracking-wider"
...
function ModuleProgressRow({
  ...
  module: ModuleForm & { progress: number; completed: number; total: number; status: 'ok' | 'warn' | 'risk' };
  onClick: () => void;
}) {
  ...
  return (
    <Card pad={14} onClick={onClick}>
      <div className="flex items-center gap-3">
        <div
          className="mono flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-[10px] text-[10px] font-bold"
          ...
          <div className="mt-2.5 flex items-center gap-2.5">
            <div className="h-1.5 flex-1 overflow-hidden rounded-full" style={{ background: P.line }}>
              <div
                ...
                style={{ width: `${module.progress}%`, background: c.solid }}
              />
            </div>
            <div
              className="mono min-w-[48px] text-right text-[11px] text-ink3"
              style={{ fontFamily: MONO }}
            >
...
function AssessmentRow({ a, moduleName }: { a: AssessmentForm; moduleName: string }) {
  ...
  return (
    <Card pad={14}>
      <div className="flex items-center gap-3">
        <div
          className="flex h-[54px] w-12 shrink-0 flex-col items-center justify-center rounded-[10px]"
