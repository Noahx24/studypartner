...
import { P, MONO, toneColors, type Tone } from './tokens';
...
export function ScreenHeader({
  ...
  return (
    <div
      className="sticky top-0 z-10 bg-app"
      style={{ padding: compact ? '16px 20px 8px' : '58px 20px 14px' }}
    >
      <div className="flex items-end justify-between gap-3">
        <div className="min-w-0 flex-1">
          {subtitle && (
            <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-ink3 mono">
              {subtitle}
            </div>
          )}
...
export function Card({
  ...
  return (
    <div
      ...
      style={{ padding: pad, ...variantStyles[variant], ...style }}
    >
      {children}
    </div>
  );
...
export function SectionLabel({
  ...
  return (
    <div className="flex items-baseline justify-between px-1 pb-3 pt-6">
      ...
      {action && (
        <button
          ...
          style={{ color: P.primary, fontFamily: 'inherit' }}
        >
          {action}
        </button>
      )}
...
export function SyncPill({
  ...
  state: 'online' | 'offline' | 'syncing' | { queued: number };
  onClick?: () => void;
}) {
  let tone: Tone = 'muted';
  let label = 'Synced';
  let leading: IconName = 'cloud';
  if (state === 'offline') {
    tone = 'warn';
    label = 'Offline';
...
export function StatCell({
  ...
  return (
    <div>
      ...
      <div
        className="mono mt-0.5 text-[16px] font-bold text-ink"
        style={{ fontFamily: MONO, letterSpacing: '-0.5px', color: accent ?? P.ink }}
      >
        {value}
      </div>
    </div>
