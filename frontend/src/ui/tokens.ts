...
export type ModulePalette = typeof COLOR_CYCLE[number];

export function moduleColor(code: string): ModulePalette {
  let h = 0;
  for (let i = 0; i < code.length; i++) h = (h * 31 + code.charCodeAt(i)) >>> 0;
  return COLOR_CYCLE[h % COLOR_CYCLE.length];
}

export type Tone = 'ok' | 'warn' | 'risk' | 'muted' | 'primary' | 'lime';
