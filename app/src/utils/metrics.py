...
prometheus_client. We expose three things:
  ...
  GET /metrics  — Prometheus exposition format. Counters for request
                  totals, all in-process — no external collector. If
                  you want richer metrics later (histograms, labels),
                  swap to the real prometheus_client library.

We avoid the prometheus_client dep on purpose: it's heavy for what we
need today, and adds another thing to maintain. The hand-rolled
exposition below is correct for the simple counter use case; the
moment we want histograms or labels, switch.
"""
...
def record_request(path: str, status: int) -> None:
    """Bump the (route-template, status) counter. Called from middleware.

    Path should be the FastAPI route template (`/modules/{id}`), not
    the concrete URL, so cardinality stays bounded.
    """
    key = (path, status)
    with _lock:
